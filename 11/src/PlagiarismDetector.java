...
public class PlagiarismDetector {
	...
	public static Map<String, Integer> detectPlagiarism(String dirName, int windowSize, int threshold) {
		...
		List<String> files = new ArrayList(Arrays.asList(directory.list()));
		String path = dirName + "/";
		...
		while (iter.hasNext()) {
			...
			Set<String> currentPhrases = createPhrases(path + currentFile, windowSize);
			if (currentPhrases == null) {
				return null;
			}

			Iterator<String> offsetIter = files.listIterator(iter.nextIndex());
			while (offsetIter.hasNext()) {
				String nextFile = offsetIter.next();
				if (currentFile.equals(nextFile)) {
					continue;
				}

				Set<String> nextPhrases = createPhrases(path + nextFile, windowSize);
				if (nextPhrases == null) {
					return null;
				}

				Set<String> matches = findMatches(currentPhrases, nextPhrases);
				int matchesSize = matches.size();
				if (matchesSize <= threshold) {
					continue;
				}

				String pair = currentFile + "-" + nextFile;
				numberOfMatches.put(pair, matchesSize);
			}
		}
